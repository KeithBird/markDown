...
$$
i=I_msin(\omega t + \varphi)=\frac{I_m}{\sqrt2}(cos\varphi+jsin\varphi)=a+bj
$$

$$
i=a+bj=\sqrt2 \cdot \sqrt{(a^2+b^2)}sin(\omega t+\theta)~~~(tan\theta=\frac{b}{a})
$$
...
**经过$R$**：$\dot{U}_R$与$\dot {I}$相向

**经过$L$**：$\dot{U}_L$与$\dot {I}$超前90度(感压超前)

**经过$C$**：$\dot{U}_C$与$\dot {I}$滞后90度(容压滞后)
...
$$
S_N=\sqrt{P^2+Q^2}~~~~~\overline S=P+jQ
$$
...
1. 无输入信号电压时，三极管各电极的电压和电流都是恒定的：
$$

$$
2. 加速输入信号电压后，都在直流量的基础上叠加了一个交流量
3. 输入电压和输出电压相位相差180度
